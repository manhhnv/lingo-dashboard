...
const ListWordQuestions = ({
  questions,
  sentenceQuestions,
}: ListWordQuestionsProps) => {
  return (
    <>
      {questions &&
        questions.map((question, index) => {
          if (question.code !== QuestionTypeCode.W9) {
            return <WordQuestion {...question} key={index} />;
          } else {
            return <MatchingQuestion {...question} key={index} />;
          }
        })}
      {sentenceQuestions &&
        sentenceQuestions.map(
          (question: MappedSentenceQuestion | undefined, index: number) => {
            if (question) {
              return <SentenceQuestions {...question} key={index} />;
            }
            return null;
          }
        )}
